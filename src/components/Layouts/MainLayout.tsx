...
function Sidebar() {
  ...
  return (
    <div className="hidden lg:flex relative w-44 bg-[#00BA9F] text-white rounded-tl-xl rounded-bl-xl min-h-screen">
      <img src={asideImage} alt="aside-typography" className="absolute top-0 left-0 w-full h-full object-cover" />
      ...
      >
        ...
        <span className={activeTab === 'voice' ? 'text-white' : 'text-white'}>تبدیل گفتار</span>
      </Link>
      <Link
        to="/archive"
      ...
      >
        ...
        <span className={activeTab === 'archive' ? 'text-white' : 'text-white'}>آرشیو</span>
      </Link>
    </div>
  );
}
...
function Header() {
  ...
  return (
    <div className="absolute left-11 top-10">
      ...
      >
        <div onClick={() => setOpen((prev) => !prev)} className="flex items-center justify-between w-full h-[37px] cursor-pointer">
          <div className="flex items-center gap-1">
            <UserIcon className="w-4 h-4" style={{ color: greenColor }} />
            <span className="text-[15px] font-normal" style={{ color: greenColor }}>مهمان</span>
          </div>
          {open ? <FilledTriangleUp className="w-3 h-2" /> : <FilledTriangleDown className="w-3 h-2" />}
        </div>
        ...
        {open && (
          <>
            ...
            />
            <div className="flex items-center gap-1 w-full h-[37px] cursor-pointer rounded-b-[20px]" style={{ color: greenColor }}>
